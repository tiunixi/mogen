<template>
	<view  :class="{'uni-scroll-tab': scroll === true }" class="uni-tab">
		...
		class="uni-tab-item">
			...
			<image :src=icon(index)  :class="[index?'wx':'zfb']"></image>
			<text class="tab-text">{{rangeKey == '' ? tab : tab[rangeKey]}}</text>
		</view>
		<view v-if="!scroll" :style="[{ right: barRight + '%', left : barLeft + '%', borderColor: activeColor }]" class="uni-tab-bar" :class="back ? 'uni-tab-bar-backward' : 'uni-tab-bar-forward'">
		</view>
	</view>
</template>
<script>
